...
| `cadence-packaging` | 两组以上“短动作—对白—解释结论”以相同节拍复现 | 阅读反应被预包装，场景像提纲逐项落地 | 改变信息抵达方式，让动作或他人反应直接造成后果 |
...
1. 记录初审正文 SHA-256。
2. 先处理重复审计：没有功能的重复应修订；确需保留的每个 exact/near finding 都要写入哈希绑定的 `repetitionExceptions`，给出具体编辑理由。再顺读并按十一类模式列证据簇；每项必须说明 `readerCost` 和只影响命中段的 `direction`。
