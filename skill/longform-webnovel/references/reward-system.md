...
本节仅适用于 `serial`。`fanqie-short-story` 不执行 3/5/15 章周期，改按 [length-modes.md](length-modes.md) 的全文比例结构定位回报。
...
规划时先用 `plan_cadence.py --write` 创建锚点，再把 `unassigned` 和“待规划”替换成具体方案。长篇普通章、短故事非锚点分节也必须有推进或回报，但不必写入节拍账本。
