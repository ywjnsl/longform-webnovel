...
- `series-map.md`：可收束主线、潜在阶段和远期选择，不是固定总纲。
- `current-volume.md`：当前卷承诺、变化终点、主要对抗和卷末判断点。
- `rolling-outline.md`：未来 5–10 章；越近越具体，越远越可调整。
...
`project.json` 是提交索引。`lastCommittedChapter` 只能指向正文和状态都已完成的章。`latestDraftChapter` 可以领先，表示存在未提交正文。
...
`project.json.rewardCadence` 保存节拍：`smallEvery: 3`、`majorEvery: 5`、`supercycle: 15`、`overlapPolicy: "major-absorbs-small"` 和开始强制执行的章号。改变节拍属于故事合同级决定，必须由作者确认。

`rewards.json` 的完整结构、证据规则和 15 章节拍见 [reward-system.md](reward-system.md)。每 3 章需要 `small` 以上兑现，每 5 章需要 `major` 兑现；重合章只保留一个 `major` 记录。
